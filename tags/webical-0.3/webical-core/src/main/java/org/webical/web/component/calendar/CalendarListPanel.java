...
public abstract class CalendarListPanel extends AbstractBasePanel {
	...
	public void setupAccessibleComponents() {
		addCalendarLink = new Link(ADD_CALENDAR_LINK_MARKUP_ID){
			...
			public void onClick() {
				...
				CalendarListPanel.this.selectCalendarForEdit(new Calendar(), null);
			}
		};
	...
	public void setupNonAccessibleComponents() {
		addCalendarLink = new IndicatingAjaxFallbackLink(ADD_CALENDAR_LINK_MARKUP_ID){
			private static final long serialVersionUID = 1L;
			...
			public void onClick(AjaxRequestTarget target) {
				CalendarListPanel.this.selectCalendarForEdit(new Calendar(), target);
			}
		};
		addOrReplace(addCalendarLink);
	}
	...
	/**
	 * Notify the parent a calendar is selected for editing
	 * @param calendar The calendar to edit
	 * @param target The Ajax Target of the panel
	 */
	public abstract void selectCalendarForEdit(Calendar calendar, AjaxRequestTarget target);
	...
	/**
	 * Notify the parent to switch the visibility of the given calendar
	 * @param calendar The calendar to switch
	 * @param target The Ajax Target of the panel
	 */
	public abstract void switchCalendarVisibility(Calendar calendar, AjaxRequestTarget target);
	...
	private class CalendarListview extends ListView {
		...
		protected void populateItem(ListItem listItem) {
			...
			Link calendarVisibleLink = new Link(CALENDAR_VISIBLE_LINK_MARKUP_ID) {
				...
				@Override
				public void onClick() {
					CalendarListPanel.this.switchCalendarVisibility(calendar, null);
				}
			};
			...
			Link calendarLink = new Link(CALENDAR_LINK_MARKUP_ID){
				...
				@Override
				public void onClick() {
					CalendarListPanel.this.selectCalendarForEdit(calendar, null);
				}
			};
