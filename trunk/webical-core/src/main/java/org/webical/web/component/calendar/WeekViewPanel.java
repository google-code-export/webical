...
public abstract class WeekViewPanel extends CalendarViewPanel {
	...
	public WeekViewPanel(String markupId, int daysToShow, GregorianCalendar currentDate) {
		...
		Date startDate = null;
		if (weekView) {
			startDate = CalendarUtils.getFirstDayOfWeek(getViewCurrentDate().getTime(), getFirstDayOfWeek());
		} else {
		...
		GregorianCalendar endCal = CalendarUtils.newTodayCalendar(getFirstDayOfWeek());
